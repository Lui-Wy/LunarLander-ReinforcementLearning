...
def draw_screen(screen, font, header_font, game_state):
    ...
    if not lander.is_alive:
        draw_centered_text(screen, header_font, "CRASH! Drücke R (PC) oder auf den Header (Handy) für Reset", RED)

    elif lander.has_landed:
        draw_centered_text(
            screen,
            header_font,
            f"SUCCESS! Zeit: {game_state.flight_time:.2f}s - Drücke R (PC) oder auf den Header (Handy) für Reset",
            GREEN
        )
